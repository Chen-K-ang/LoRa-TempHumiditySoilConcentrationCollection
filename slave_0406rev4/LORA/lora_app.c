...
//各类指标限制，超过则报警
#define HUM_LIMIT 20.0f
#define N_LIMIT   1.0f
#define P_LIMIT   1.0f
#define K_LIMIT   1.0f

static void compare_val(void)
{
	if (lora_rx_data.humidity.f_data > HUM_LIMIT)
		HUM_BIT = 1;
	if (lora_rx_data.N_val.f_data > N_LIMIT
		|| lora_rx_data.P_val.f_data > P_LIMIT
		|| lora_rx_data.K_val.f_data > K_LIMIT)
		NPK_BIT = 1;
}

soil_data lora_rx_data;

u8 rlcd_buff[10]={0}; //LCD显示字符串缓冲区
u8 humidity_buff[20]={0};
u8 N_val_buff[20]={0};
u8 P_val_buff[20]={0};
u8 K_val_buff[20]={0};
...
void LoRa_ReceData(void)
{
	...
	if(USART3_RX_STA&0x8000)
	{
		...
		for (i = 0; i < len; i++) {
			if (i < 4)
				lora_rx_data.temp.byte[i] = USART3_RX_BUF[i+1];
			while(USART_GetFlagStatus(USART1, USART_FLAG_TC) == RESET); //循环发送,直到发送完毕   
			USART_SendData(USART1, USART3_RX_BUF[i]); 
		}
		
		LCD_Fill(10,10,240,320,WHITE);
		if(LoRa_CFG.mode_sta == LORA_STA_Tran)//透明传输
		{	
			if((USART3_RX_BUF[0]==0xaa)&&(USART3_RX_BUF[17]==0x55)) {
				for (i = 0; i < 4; i++) {
					lora_rx_data.humidity.byte[i] = USART3_RX_BUF[1 + i];
					lora_rx_data.N_val.byte[i]    = USART3_RX_BUF[1 + i + F_DATALEN];
					lora_rx_data.P_val.byte[i]    = USART3_RX_BUF[1 + i + F_DATALEN * 2];
					lora_rx_data.K_val.byte[i]    = USART3_RX_BUF[1 + i + F_DATALEN * 3];
				}
				compare_val(); //标定高电平
				sprintf((char*)humidity_buff, "hum:%.2f%%", lora_rx_data.humidity.f_data);
				Show_Str_Mid(15,100,humidity_buff,16,240);
				sprintf((char*)N_val_buff, "N:%.2fmol/L", lora_rx_data.N_val.f_data);
				Show_Str_Mid(15,80,N_val_buff,16,240);
				sprintf((char*)P_val_buff, "P:%.2fmol/L", lora_rx_data.P_val.f_data);
				Show_Str_Mid(15,60,P_val_buff,16,240);
				sprintf((char*)K_val_buff, "K:%.2fmol/L", lora_rx_data.K_val.f_data);	
				Show_Str_Mid(15,40,K_val_buff,16,240);
			}
		}
		else if(LoRa_CFG.mode_sta==LORA_STA_Dire)//定向传输
		{
			//将十六进制的数据转化为字符串打印在lcd_buff数组
			sprintf((char*)rlcd_buff, "%f", lora_rx_data.temp.f_data);
