...
void NET_IO_AddTag2End(void)
{
	...
	DMA_SetCurrDataCounter(DMA1_Channel3, RB_BUFFER_SIZE - net_io_info.write_pos);	//重新设置剩余字节
	DMA1_Channel3->CMAR = (unsigned int)(net_io_info.buf + net_io_info.write_pos);	//重新设置当前DMA指向内存的指针
	
	DMA_Cmd(DMA1_Channel3, ENABLE);													//使能MDA
#endif
...
void USART3_IRQHandler(void)
{
	...
	if(USART_GetFlagStatus(USART3, USART_FLAG_RXNE) != RESET)			//接收中断
	{
		...
		net_io_info.buf[net_io_info.write_pos++] = USART3->DR;				//写入rb
